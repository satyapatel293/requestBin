...
const getSingleBasketRequests = async (
  basket_id: string
): Promise<Requests[]> => {
  try {
    const result = await client.query(
      `SELECT * FROM requests WHERE basket_id = $1`, [basket_id]
    );
...
const deleteBasket = async (basket_id: string): Promise<string> => {
  try {
    const result = await client.query(
      `DELETE FROM requests WHERE basket_id = $1`, [basket_id]
    );
    const result2 = await client.query(
      `DELETE FROM baskets WHERE basket_name = $1`, [basket_id]
    );
...
const addRequest = async (requestObj: NewRequest): Promise<string> => {
  try {
      await client.query(`INSERT INTO requests (id, basket_id, path, method, headers)
VALUES ($1, $2, $3, $4, $5)`, [requestObj.id, requestObj.basket_id, requestObj.path, requestObj.method, requestObj.headers]);
...
const addNewBasket = async (basket_id: string): Promise<string> => {
  try {
    const result = await client.query(
      `INSERT INTO baskets VALUES ($1)`, [basket_id]
    );
